...
func (c Client) UpdateTestResults(
	...
	for _, strip := range []stripFunc{
		func(tr v1.TestResults) v1.TestResults {
			c.Log.Warnf("removing original test result data from uploaded Captain test results due to content size threshold")
			return v1.StripDerivedFrom(tr)
		},
		func(tr v1.TestResults) v1.TestResults {
			c.Log.Warnf(
				"removing previous test attempt backtraces from uploaded Captain test results due to content size threshold",
			)
			return v1.StripPreviousAttempts(tr)
		},
		func(tr v1.TestResults) v1.TestResults {
			c.Log.Warnf(
				"removing current test attempt backtraces from uploaded Captain test results due to content size threshold",
			)
			return v1.StripCurrentAttempts(tr)
		},
	} {
